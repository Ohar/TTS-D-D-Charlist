...
function updateLevelByExp()
    ...
    if (exp < EXP_MIN or exp > EXP_MAX) then
        ...
        Wait.time(function()
            self.editInput({ index = EXP_INPUT_EDIT_ID, value = exp })
        end, 0)
    end
...
function changeHitDiceLeft(addDice)
    ...
    if (nextHitDiceLeft < HIT_DICE_MIN or nextHitDiceLeft > HIT_DICE_MAX) then
        if (nextHitDiceLeft < HIT_DICE_MIN) then
            nextHitDiceLeft = HIT_DICE_MIN
        end
        if (nextHitDiceLeft > HIT_DICE_MAX) then
            nextHitDiceLeft = HIT_DICE_MAX
        end
    end
